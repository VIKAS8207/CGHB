...
import { 
  ...
  Trash2, AlertCircle, MoreVertical, MapPin, ArrowLeft, HardHat, ShieldCheck
} from 'lucide-react';
...
// Mock Data (Updated with Designation and District)
const initialEngineers = [
  { id: 'EMP-8021', name: 'Rajesh Sharma', designation: 'Assistant Engineer (AE)', district: 'Raipur', phone: '+91 98765 43210', email: 'rajesh.s@cghb.gov.in' },
  { id: 'EMP-8022', name: 'Priya Patel', designation: 'Sub Engineer (SE)', district: 'Bilaspur', phone: '+91 98765 43211', email: 'priya.p@cghb.gov.in' },
  { id: 'EMP-8034', name: 'Amit Kumar', designation: 'Sub Engineer (SE)', district: 'Durg', phone: '+91 98765 43212', email: 'amit.k@cghb.gov.in' },
  { id: 'EMP-8045', name: 'Suresh Iyer', designation: 'Assistant Engineer (AE)', district: 'Bastar', phone: '+91 98765 43213', email: 'suresh.i@cghb.gov.in' },
];
...
const Engineers = () => {
  ...
  const handleEdit = (eng) => {
    setEditingId(eng.id);
    setFormData({
      empId: eng.id, name: eng.name, designation: eng.designation, district: eng.district, phone: eng.phone, email: eng.email
    });
    setIsFormOpen(true);
  ...
  const filteredEngineers = engineers.filter(eng => 
    ...
    eng.district.toLowerCase().includes(searchTerm.toLowerCase())
  );
          ...
          <div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
              ...
              <div className="bg-[var(--color-bg-main)] p-4 rounded-lg border border-cghb-border/50 shadow-sm">
                ...
                <span className="block text-[15px] font-black text-[var(--color-text-main)]">{viewingEngineer.designation || '-'}</span>
              </div>
            </div>
          </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
              <div className="flex items-center gap-4 bg-[var(--color-bg-main)] p-4 rounded-lg border border-cghb-border/50 shadow-sm">
                ...
                <div>
                  ...
                  <span className="block text-[14px] font-bold text-[var(--color-text-main)]">{viewingEngineer.district || '-'}</span>
                </div>
              </div>
              <div className="flex items-center gap-4 bg-[var(--color-bg-main)] p-4 rounded-lg border border-cghb-border/50 shadow-sm">
              ...
              <div className="flex items-center gap-4 bg-[var(--color-bg-main)] p-4 rounded-lg border border-cghb-border/50 shadow-sm">
                ...
                <div>
                  ...
                  <span className="block text-[14px] font-medium text-[var(--color-text-main)] truncate max-w-[150px]" title={viewingEngineer.email}>{viewingEngineer.email || '-'}</span>
                </div>
              </div>
            </div>
          </div>
    ...
    <div className="w-full max-w-[1400px] mx-auto animate-in fade-in duration-300 font-sans relative z-10 space-y-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        ...
        <div className="glass-panel p-5 rounded-xl border-t-4 border-t-blue-500 flex items-center justify-between">
          <div>
            <p className="text-[11px] font-bold text-[var(--color-text-muted)] uppercase tracking-wider mb-1">Active Districts</p>
            <h3 className="text-3xl font-black text-[var(--color-text-main)]">
              {new Set(engineers.map(e => e.district)).size}
            </h3>
          </div>
        ...
        <div className="glass-panel p-5 rounded-xl border-t-4 border-t-emerald-500 flex items-center justify-between">
          <div>
            <p className="text-[11px] font-bold text-[var(--color-text-muted)] uppercase tracking-wider mb-1">Assistant Engineers</p>
            <h3 className="text-3xl font-black text-[var(--color-text-main)]">
              {engineers.filter(e => e.designation.includes('AE')).length}
            </h3>
          </div>
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-3 gap-5">
                    ...
                    <div>
                      ...
                      <select required value={formData.designation} onChange={e => setFormData({...formData, designation: e.target.value})} className="w-full h-11 bg-[var(--color-bg-surface)] border border-cghb-border text-[var(--color-text-main)] text-[13px] rounded-lg px-4 focus:outline-none focus:border-cghb-yellow focus:ring-1 focus:ring-cghb-yellow transition-all shadow-sm cursor-pointer">
                        ...
                        <option value="Assistant Engineer (AE)" className="text-black">Assistant Engineer (AE)</option>
                        <option value="Sub Engineer (SE)" className="text-black">Sub Engineer (SE)</option>
                      </select>
                    </div>
              ...
              <AnimatePresence>
                {currentEngineers.map((eng, index) => (
                  <motion.tr layout initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} key={eng.id} className="bg-transparent border-b border-cghb-border/50 last:border-0 hover:bg-cghb-border/5 transition-colors">
                    ...
                    <td className="px-4 py-4 text-[12px] font-medium text-[var(--color-text-main)] truncate" title={eng.designation}>{eng.designation}</td>
                    <td className="px-4 py-4 text-[12px] font-bold text-[var(--color-text-main)] truncate" title={eng.district}>{eng.district}</td>
